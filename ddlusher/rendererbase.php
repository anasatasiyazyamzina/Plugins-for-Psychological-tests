 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlusher_renderer_base extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        $output .= html_writer:: start_tag('div',array('id'=>'countDrop','name'=> 'countDrops','style'=>'visibility: hidden'));
        $count_dragsindrops = 0;
        foreach ($question->places as $placeno => $place) {
            $varname = $question->field($placeno);
            $k = $this->inputs_values($qa, $varname);
            if($k[1]!=0) $count_dragsindrops = $count_dragsindrops+1;
           // echo $count_dragsindrops;
        }
        $output .= $count_dragsindrops;
        $output .= html_writer:: end_tag('div');

        return $output;
    }

    protected function inputs_values(question_attempt $qa, $varname, $value = null) {
        if ($value === null) {
       ...
       // print_r($attributes);
        ...
        return array($fieldname,$attributes['value']);
    }
